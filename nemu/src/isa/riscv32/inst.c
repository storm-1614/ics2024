...
/* 按 type 规定的位布局，从机器码里把所有数值取出来，填进指针参数
 */
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                                                           \
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, R(rd) = s->pc + 4; s->dnpc = src1 + imm;
            IFDEF(CONFIG_FTRACE, if (rd != 0) ftrace_call(s->pc, s->dnpc); else ftrace_ret(s->pc)));
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm;
            IFDEF(CONFIG_FTRACE, if (rd != 0) ftrace_call(s->pc, s->dnpc)));
    ...
    INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui, U, R(rd) = imm); // immU 自带左移 12
    INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli, I, R(rd) = src1 >> imm);
    INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli, I, R(rd) = src1 << imm);
    INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu, B, if (src1 >= src2) s->dnpc = s->pc + (sword_t)imm);
    ...
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R,
            R(rd) = (sword_t)((int64_t)(sword_t)src1 * (int64_t)(sword_t)src2 >> 32));
    INSTPAT("0000001 ????? ????? 111 ????? 01100 11", rem, R, R(rd) = src1 % src2);
    INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra, R, R(rd) = (sword_t)src1 >> src2);
    INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl, R, R(rd) = src1 >> src2);
    INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R, R(rd) = ((u_int64_t)src1 * (u_int64_t)src2) >> 32);
    INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B, if (src1 < src2) s->dnpc = s->pc + (sword_t)imm);
